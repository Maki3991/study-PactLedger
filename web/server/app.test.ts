...
test('auth flow: register, login, me, logout and access control', async () => {
...
test('tasks are isolated per user', async () => {
  ...
  const ownRead = await app.inject({ method: 'GET', url: `/api/tasks/${task.id}`, headers: auth(aliceToken) })
  assert.equal(ownRead.statusCode, 200)

  const foreignRead = await app.inject({ method: 'GET', url: `/api/tasks/${task.id}`, headers: auth(bobToken) })
  assert.equal(foreignRead.statusCode, 404)

  const foreignApprove = await app.inject({ method: 'POST', url: `/api/tasks/${task.id}/approve`, headers: auth(bobToken) })
  assert.equal(foreignApprove.statusCode, 404)

  const foreignTreasury = await app.inject({ method: 'GET', url: `/api/treasury/${task.id}/accounts`, headers: auth(bobToken) })
  assert.equal(foreignTreasury.statusCode, 404)
