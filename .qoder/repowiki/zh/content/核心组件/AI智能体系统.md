...
## 更新摘要
**所做更改**   
- 移除了AI智能体系统和DeepSeek集成的相关文档内容（server/src/ai目录已删除）
- 更新了项目结构图，移除AI相关组件
- 标记AI功能为废弃状态
- 保留了Web端任务编排和金库管理功能的文档
...
## 简介
本仓库实现了一个基于Web的任务编排与金库管理系统。**注意：原AI智能体系统和DeepSeek集成功能已被完全移除**。系统现在主要围绕"任务驱动的执行编排"展开：通过API创建与管理任务，协调执行流程，持久化状态到数据库，并通过金库模块管理链上资产与交易。Web端提供任务流、执行视图与金库管理等界面，便于用户观察与控制整个流程。

## 项目结构
整体采用前后端分离的架构：
- server：后端服务，负责HTTP路由、数据库访问、任务编排与金库服务。
...
**注意**：原AI智能体和DeepSeek客户端功能已被移除，不再包含在系统中。
...
- 后端服务提供REST API，路由层调用数据库，持久化任务与结果。
...
## 依赖关系分析
- 低耦合高内聚：路由层仅关注HTTP契约，业务逻辑下沉至服务层与金库模块。
...
## 结论
该任务编排系统以任务为核心，结合金库操作，形成端到端的自动化执行链路。通过清晰的分层与适配器模式，系统在可扩展性与可维护性方面具备良好基础。**注意：原AI智能体系统和DeepSeek集成功能已被完全移除，如需重新实现AI功能，需要在web/server中重新开发相应模块**。建议在生产环境中完善监控告警、幂等设计与审计日志，进一步提升稳定性与可观测性。
