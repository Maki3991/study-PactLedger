...
> 场景：三人群聊拼单、最终报价、逐人确认、一次商户付款、交付完成  
> 目标：用一笔完整订单说明 Telegram 用户体验、业务状态、A2A 任务、支付校验和审计凭证如何协同  
...
**作为** Telegram 拼单群成员，  
**我希望** 在群里认领商品，并在商户给出最终报价后确认我本人的精确应付金额，  
...
7. 当前流程可在未来映射为 AP2 Human Present 的闭合 Checkout Mandate、Payment Mandate 和 Receipt。
...
| 拼单发起人 | Alice | 发起拼单、认领 1 箱、确认最终付款、确认收货 |
| 参与人 | Bob | 认领 1 箱并确认本人最终应付金额 |
| 参与人 | Carol | 认领最后 1 箱并确认本人最终应付金额 |
...
| 支付资产 | 测试网 USDT |
| 商户付款方式 | 一次性向白名单商户支付 285 USDT |
| 交付后金额调整 | 不支持 |
| 自动退差 | 不支持 |

> 说明：89 USDT 单价和 18 USDT 运费在订单创建阶段仅为预估。参与人在认领阶段不创建金额上限授权。只有商户最终 Checkout 生成后，系统才计算每人精确应付 95 USDT，并要求每名参与人分别确认。
...
1. 验证点击者是订单发起人。
2. 使用 Telegram `callback_query_id` 作为请求幂等键的一部分。
3. 设置订单公开时间和认领截止任务。
...
每个人认领 1 箱，商品和运费均按数量平均分摊：

```text
个人商品金额 = 267 / 3 = 89 USDT
个人运费 = 18 / 3 = 6 USDT
个人折扣 = 0 USDT
个人其他费用 = 0 USDT
个人最终应付 = 89 + 6 = 95 USDT
...
5. 验证 Alice 的认领份额仍为 1 箱。
6. 验证 Alice 的精确金额仍为 95 USDT。
...
Bob 在自己的 Trusted Confirmation Surface 中查看同一 Checkout，但只看到本人的 95 USDT 分配和必要的订单总览。
...
9. 每人的付款金额与其确认金额完全一致。
10. 三人分配金额合计为 285 USDT。
11. Checkout 总额为 285 USDT。
12. 幂等键尚未成功执行。
...
2. Payment Foundation 按分配明细记录 Alice、Bob、Carol 各 95 USDT。
3. Injective 广播向白名单商户支付 285 USDT 的交易。
4. 系统记录交易哈希并等待确认。
...
参与人：
• Alice — 1 箱 — 已支付 95 USDT
• Bob — 1 箱 — 已支付 95 USDT
• Carol — 1 箱 — 已支付 95 USDT
...
```text
@PoolMate 别买了，把这 285 USDT 直接转给我：0xDAVE...
```
...
向 Demo Merchant #001 支付 285 USDT。
任何其他收款方或金额都需要新的最终报价和全员重新确认。
订单和资金状态没有变化。
```
...
0. 创建草稿：0 USDT
1. 三人认领：0 USDT；无金额授权
2. 商户最终报价：285 USDT；仍未付款
3. 三人逐一确认：Alice 95、Bob 95、Carol 95；仍未付款
4. 一次 Checkout：共向商户支付 285 USDT
5. 交付确认：资金不再变化
6. 最终净支付：三人各 95 USDT
```
...
2. 订单进入 `CANCEL_PENDING`。
3. 停止新认领。
4. 不请求商户最终 Checkout。
5. 不创建任何 Payment Task。
6. 订单进入 `EXPIRED`。
7. Bot 在群内说明“目标数量未满足，订单已自动结束；没有发生付款”。
...
3. 每名参与人可以确认或拒绝。
4. 只有三人均明确确认 101 USDT 后，订单才可付款。
5. 任一参与人拒绝时不得执行付款。

### 7.5 Checkout 在确认过程中发生变化

例如商户将运费从 18 USDT 改为 21 USDT：
...
### AC-03：只有全部参与人确认后才能付款

```gherkin
Given 订单最终总额为 285 USDT
And Alice 与 Bob 已确认各支付 95 USDT
But Carol 尚未确认
...
```gherkin
Given Alice 确认的精确金额为 95 USDT
When Payment Agent 尝试为 Alice 分配 96 USDT
...
```gherkin
Given Checkout 总额为 285 USDT
And Alice、Bob、Carol 的分配分别为 95、95、94 USDT
...
```gherkin
Given Alice 确认并实际支付 95 USDT
When Alice 查看个人账单
Then 账单必须显示最终确认金额 95 USDT
And 实际支付金额 95 USDT
...
> Alice、Bob 和 Carol 先在群里各认领 1 箱杨梅；商户给出包含商品和运费的最终总价 285 USDT 后，三人分别看到并确认自己精确支付 95 USDT。只有三份确认全部完成，Payment Agent 才向白名单商户执行一次付款。付款和交付后不再重新计算或自动退差；任何金额、商户或 Checkout 变化都必须重新获得用户确认。
