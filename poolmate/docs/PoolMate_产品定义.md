...
2. 当前自然语言适配器只提取商品标题和目标数量，不提取商户、价格、资产、金额、收款方或截止时间。
3. 字段缺失或歧义时不创建订单；完整结果只持久化为 `DRAFT`。
4. 发起人点击 `Confirm & publish` 后，订单才进入公开认领阶段；也可以点击 `Discard draft` 留下取消证据。
...
11. 自然语言能力默认关闭，关闭或不可用时命令流程必须继续工作。
12. LLM 只允许返回 `title`、`targetUnits` 和缺失/歧义字段，额外字段必须拒绝。
