...
1. 群成员通过 `/pool_new <targetUnits> <title>`，或在群内明确 @mention Bot 后用自然语言发起拼单。
2. 当前自然语言适配器提取商品、目标数量、单位、采购渠道偏好和用户参考价；例如 `@PoolMate 拼单 3瓶可乐，美团外卖` 会保留“可乐 / 3 / 瓶 / 美团外卖”。
3. 采购渠道和参考价属于不可信用户意图，不等于已验证商户、收款方或最终价格；当前 Checkout 仍由固定 Demo Merchant 模拟生成。
4. 字段缺失或歧义时不创建订单；完整结果只持久化为 `DRAFT`。
5. 发起人点击 `Confirm & publish` 后，订单才进入公开认领阶段；也可以点击 `Discard draft` 留下取消证据。
...
12. LLM 只允许返回标题、商品、目标数量、单位、采购渠道偏好、用户参考价和缺失/歧义分类；任何 merchant、payee、asset、final amount、Checkout、确认、付款或状态字段都必须拒绝。
13. LLM 输出不得直接发布订单；只有订单发起人可以将 `DRAFT` 发布为 `COLLECTING`。
